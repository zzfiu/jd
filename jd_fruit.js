...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https:///api/farm/${randomCount}`, timeout: 10000,}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
// function submitCode() {
//   return new Promise(async resolve => {
//   $.get({url: `http:///jdcodes/submit.php?code=${$.farmInfo.farmUserPro.shareCode}&type=farm`, timeout: 10000}, (err, resp, data) => {
//     try {
//       if (err) {
//         console.log(`${JSON.stringify(err)}`)
//         console.log(`${$.name} API请求失败，请检查网路重试`)
