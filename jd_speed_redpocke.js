...
function reward_query() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_query", {
      "inviter": ["L4-TSMc1lZ80VjeiqjGmEOqQgvarWYUSGhrXbsc24Q"][Math.floor((Math.random() * 1))],
      linkId
    }), async (err, resp, data) => {
      try {
        if (err) {
...
async function redPacket() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_receive",{"inviter":["L4-TSMc1lZ80VjeiqjGmEOqQgvarWYUSGhrXbsc24Q"][Math.floor((Math.random() * 1))], linkId}),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`)
...
function invite2() {
  let inviterIdArr = [
    "L4-TSMc1lZ80VjeiqjGmEOqQgvarWYUSGhrXbsc24Q",
    "L4-TSMc1lZ80VjeiqjGmEOqQgvarWYUSGhrXbsc24Q"
  ]
