...
async function jdPet() {
  try {
    ...
    if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
      ...
      // 报告运行次数
      $.get({
        url: `http://192.168.31.101:5702/api/runTimes?activityId=pet&sharecode=${$.petInfo.shareCode}`,
        timeout: 10000
      }, (err, resp, data) => {
        if (err) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://192.168.31.101:5702/api/pet/${randomCount}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
