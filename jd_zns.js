...
$.logUrl = process.env.jd_zns_logUrl || logUrl // 优先选择此url

let openckFlag = 1 // 1开启 2不开启
$.openckFlag = process.env.jd_zns_openckFlag || openckFlag // 开启验证appck


let outuserIdArr = [];
let outuserID = '';// 屏蔽账号 2,5,7
if ($.isNode() && process.env.jd_zns_outuserID) {
  outuserID = process.env.jd_zns_outuserID;
}
...
!(async () => {
  ...
  console.log("当前版本：2023年01月04日")
  console.log("TOKEN："+$.token.replace(/.+(.{5})/, '***$1')+"\n")
  
  let blacklist = '' // 黑名单 用&隔开 pin值(填中文
  $.blacklist = process.env.jd_zns_blacklist || blacklist // 黑名单
  getBlacklist()
  let urlArr = [
    // "http://127.0.0.1",
    "http://g.zxi7.cn",
...
async function movement() {
  try {
    ...
    // await takePostRequest($.functionCode+'_customizeGamesIcon');
    // await takePostRequest($.functionCode+'_customizeGamesPrize');
    ...
    if($.homeData.result){
      ...
      if(!$.fullFlag){
        console.log(`分红:${$.raiseInfo.redInfo.red}份 金币:${$.raiseInfo.totalScore} ${$.raiseInfo.produceResult.produceScore}金币待领取 抽奖需消耗:${$.raiseInfo.scenceMap.redNum.nextLevelScore}`)
        // for(let i of $.raiseInfo.scenceMap.sceneInfo){
        //   console.log(i.name,'抽奖需消耗:'+i.redNum.nextLevelScore,i.redNum.status != 1 && '离分红还差:'+i.redNum.nextNum || '已经升满了')
        // }
        // if($.homeMainInfo.produceResult.produceScore >= 1){
        if($.raiseInfo.produceResult.produceScore >= $.raiseInfo.produceResult.produceMax*0.7){
          await takePostRequest($.functionCode+'_collectAutoScore');
        }
      }
    }
    await takePostRequest($.functionCode+'_getSignHomeData');
    let signNum = 0
    for(let i of $.signHomeData.signNodes || {}){
      if(i.status == 1) signNum++
    }
    console.log(`已累计签到${signNum}/${$.signHomeData.signNodes.length}天，还需签到${$.signHomeData.signNodes.length-signNum}天`)
    // for(let st of $.signHomeData.signNodes || []){
    //   if(st.signTime) console.log(`第${st.dayNo}天 签到时间：${$.time('MM-dd HH:mm', Number(st.signTime))}`)
    // }
    if($.signHomeData.todayStatus === 0 && $.signHomeData.signStatus === 0){
      if(!$.hotFlag) await takePostRequest($.functionCode+'_sign');
      ...
      for (let i = 0; i < $.taskList.length && !$.hotFlag; i++) {
        ...
        if([5].includes($.oneTask.taskType) && $.oneTask.status === 1 && $.oneTask.taskName.includes("种草")){
          ...
          for (let j = 0; j < $.activityInfoList.length; j++) {
            ...
            console.log(`做任务：${(taskName).substring(0,10)} ${$.oneTask.waitDuration > 0 && '等待'+$.oneTask.waitDuration+'秒'||''}`);
        ...
        }else if($.oneTask.status === 3 || $.oneTask.taskName.includes("去首页") || $.oneTask.taskName.includes("打卡") || $.oneTask.taskName.includes("去APP") || $.oneTask.taskName.includes("去品牌墙浏览") || ($.oneTask.taskName.includes("去逛逛并下单") && $.oneTask.status == 1)){
          $.taskStatus = true
          $.taskId = $.oneTask.taskId
    ...
    if($.lotteryTaskVos && $.lotteryTaskVos[0]){
      console.log(`累计任务奖励-当前进度${$.lotteryTaskVos[0].times}/${$.lotteryTaskVos[0].maxTimes}`)
      for (let i = 0; i < $.lotteryTaskVos[0].badgeAwardVos.length && !$.hotFlag; i++) {
        $.oneTask = $.lotteryTaskVos[0].badgeAwardVos[i];
        if($.oneTask.status == 3){
          $.awardToken = $.oneTask.awardToken
          await takePostRequest($.functionCode+'_getBadgeAward');
          await $.wait(getRndInteger(2000, 4000));
        }
        if($.hotFlag) break
      }
    ...
    for (let i = 0; i < $.shopTaskList.length; i++) {
      ...
      for (let j = 0; j < $.activityInfoList.length; j++) {
        ...
        if($.oneActivityInfo.status == 1){
          ...
          $.collectScore = true
          $.callbackInfo = {}
          console.log($.oneActivityInfo.shopName)
    ...
    do{
      $.shareFlag = false
      shareCount++
      if(!$.hotFlag) await takePostRequest($.functionCode+'_getWelfareScore');
      await $.wait(getRndInteger(2000, 2500));
    }while (!$.hotFlag && $.shareFlag && shareCount < 10)
    ...
    do{
      ...
      if(!$.hotFlag) await takePostRequest($.functionCode+'_getWelfareScore');
      await $.wait(getRndInteger(2000, 2500));
    }while (!$.hotFlag && $.shareFlag && shareCount < 20)
    // */
    ...
    if($.homeData.result){
      ...
      $.toTaskFlag = true
      $.raiseFlag = true
      $.scenceMap = $.toObj($.toStr($.raiseInfo.scenceMap.sceneInfo))
      // let update = updateScenceMap(false)
      console.log()
      $.nextLevelScore = parseInt($.raiseInfo.scenceMap.redNum.nextLevelScore)
      
      while ($.raiseInfo.totalScore >= $.nextLevelScore && $.toTaskFlag && $.raiseFlag) {
        console.log(`当前金币：${$.raiseInfo.totalScore} 抽奖需消耗:${$.nextLevelScore}`)
        // $.scenceId = update['scenceId']
        $.raiseFlag = false
        await takePostRequest($.functionCode+'_raise');
        await takePostRequest($.functionCode+'_getHomeData');
        $.homeMainInfo = $.homeData.result.homeMainInfo
        $.raiseInfo = $.homeMainInfo.raiseInfo
        if($.raiseFlag){
          // update = updateScenceMap(false)
          $.nextLevelScore = parseInt($.raiseInfo.scenceMap.redNum.nextLevelScore)
          // $.scenceMap = $.toObj($.toStr($.raiseInfo.scenceMap.sceneInfo))
        }else{
          console.log()
      ...
      // $.scenceMap = $.toObj($.toStr($.raiseInfo.scenceMap.sceneInfo))
      // update = updateScenceMap(true)
      console.log(`分红:${$.raiseInfo.redInfo.red}份 金币:${$.raiseInfo.totalScore} ${$.raiseInfo.produceResult.produceScore}金币待领取  抽奖需消耗:${$.nextLevelScore}`)
...
function updateScenceMap(vsconsole = false) {
  let status = false
  for (let i in $.scenceMap) {
    let v = $.scenceMap[i]
    if (vsconsole) console.log("\n「" + v.name + "」", v.status == 0 ? '未开启' : v.status != 1 ? '全解锁' : '抽奖需要' + v.redNum.nextLevelScore + '金币' + " 离分红还差："+ v.redNum.nextNum)
    if (v.status == 1) {
      if (!status || (v.redNum.nextNum == 1 && status.redNum.nextNum == 1 && status.redNum.nextLevelScore*1 > v.redNum.nextLevelScore*1) || (v.redNum.nextNum == 1 && status.redNum.nextNum != 1) || (status.redNum.nextNum != 1 && status.redNum.nextLevelScore*1 > v.redNum.nextLevelScore*1)) {
        status = v
      }
      if (vsconsole) {
        let list1 = []
        let list2 = []
        for (let is in v.specialCardList) {
          let vs = $.scenceMap[i].specialCardList[is]
          if (vs.status == 1) {
            list1.push(vs.name)
          } else {
            list2.push(vs.name)
        ...
        if(list2.length == 0){
          ...
          console.log("\t已解锁：" + list1.join("｜"))
          console.log("\t未解锁：" + list2.join("｜"))
        }
        list1 = []
        list2 = []
        for (let is in v.cardList) {
          let vs = $.scenceMap[i].cardList[is]
          if (vs.status == 1) {
            list1.push(vs.name)
          } else {
            list2.push(vs.name)
...
async function takePostRequest(type) {
  ...
  switch (type) {
    ...
    case 'getEncryptedPinColor':
      client = 'wh5'
      clientVersion = '1.0.0'
      body = `functionId=getEncryptedPinColor&client=${client}&clientVersion=1.0.0&body={}`;
    ...
    case 'qryCompositeMaterials':
      ...
      body = `functionId=qryCompositeMaterials&client=wh5&clientVersion=1.0.0&body=${$.toStr(arr,arr)}`;
      myRequest = await getPostRequest(type, body);
      break;
    ...
    case 'followShop':
      client = 'wh5'
      clientVersion = '1.0.0'
      body = `functionId=followShop&body={"shopId":"${$.oneActivityInfo.shopId}","follow":true,"type":"0"}&client=wh5&clientVersion=${clientVersion}`;
      myRequest = await getPostRequest(type, body);
      break;
...
async function dealReturn(type, res) {
  ...
  switch (type) {
    ...
    case $.functionCode+'_getWelfareScore':
      if (data.code === 0 && data.data && data.data.result) {
        if(data.data.result.maxTimes > data.data.result.times){
          console.log(`获得: ${data.data.result.score}金币`)
          $.shareFlag = true
        }else{
          console.log("分享已达上限")
        }
    ...
    case $.functionCode+'_raise':
      ...
      case $.functionCode+'_getBadgeAward':
        if(data.data && data.data.bizCode === 0){
          var myAwardVos = data.data.result.myAwardVos
          var list = {
            "15":"金币",
            "1":"优惠券",
          }
          ...
          for(let i of myAwardVos){
              console.log("获得",list[i.type] || '',names[i.type] && i[names[i.type]].score || (i.type == 1 && i[names[i.type]].usageThreshold+"-"+i[names[i.type]].quota+" "+i[names[i.type]].useRange) || i)
          }
      ...
      if(data.data && data.data.bizCode === 0){
        ...
        if(!data.data.result.taskToken){
          ...
          if(data.data.result.score || data.data.result.rewardScore || false) {
            msg += ` ${data.data.result.score || data.data.result.rewardScore}金币`
          }
  ...
  return new Promise(async resolve => {
    ...
    try {
      ...
      if (type === 'help' || type === 'shHelp') {
        taskBody = `functionId=${$.functionCode}_collectScore&client=wh5&clientVersion=-1&appid=signed_wh5&body=${JSON.stringify(await getLogs(`${$.functionCode}_collectScore`, {"actionType":"0","inviteId":$.inviteId}))}`;
      } else if (type === $.functionCode+'_sign' || type === $.functionCode+'_collectAutoScore') {
        taskBody = `functionId=${type}&client=wh5&clientVersion=-1&appid=signed_wh5&body=${JSON.stringify(await getLogs(type, { }))}`;
      } else if (type === $.functionCode+'_raise') {
        taskBody = `functionId=${type}&client=wh5&clientVersion=-1&appid=signed_wh5&body=${JSON.stringify(await getLogs(type, {"scenceId":$.scenceId}))}`;
      } else if (type === $.functionCode+'_startTraining' || type === $.functionCode+'_speedTraining') {
        taskBody = `functionId=${type}&client=wh5&clientVersion=-1&appid=signed_wh5&body=${JSON.stringify(await getLogs(type, { }))}`;
      } else if (type === $.functionCode+'_customizeGamesIcon') {
        taskBody = `functionId=${type}&client=wh5&clientVersion=-1&appid=signed_wh5&body=${JSON.stringify(await getLogs(type, {"appId":"1DFVRwqk","appSign":"1"}))}`;
      } else if (type === $.functionCode+'_getMainMsgPopUp') {
        taskBody = `functionId=${type}&client=wh5&clientVersion=-1&appid=signed_wh5&body=${JSON.stringify(await getLogs(type, {"channel":"1"}))}`;
      } else if(type === 'add_car'){
        taskBody = `functionId=${$.functionCode}_collectScore&client=wh5&clientVersion=-1&appid=signed_wh5&body=${JSON.stringify(await getLogs(`${$.functionCode}_collectScore`,{"taskId": $.taskId,"taskToken" : $.taskToken}))}`
      }else{
        let body = {"taskId": $.oneTask.taskId,"taskToken" : $.oneActivityInfo.taskToken}
        ...
        body = await getLogs(type, body)
        taskBody = `functionId=${type}&client=wh5&clientVersion=-1&appid=signed_wh5&body=${JSON.stringify(body)}`
      }
...
async function getLogs(functionId,body = {}){
  ...
  let joyytokenb = ''
  if(!$.joyytoken){
    $.joyytoken = await gettoken("50174")
  }
  joyytoken = $.joyytoken
  if(!$.joyytokenb){
  ...
  let resBody = {"fn":"zns","id":functionId,"riskData":'',"pin":$.UserName,"joyytoken":joyytoken,"uid":$.uid || "","joyytokenb":joyytokenb}
  if(!$.cookie.includes("pt_key=app_open")){
    resBody["m"] = 1
  }
...
function gettoken(appname) {
  return new Promise(resolve => {
    let body = `content={"appname":"50174","whwswswws":"","jdkey":"a","body":{"platform":"1"}}`
    if(appname != "50174"){
...
async function getPostRequest(type, body) {
  ...
  const headers = {
    ...
    'Cookie': `${$.joyytoken && 'joyytokem=50174'+$.joyytoken+'; ' || ''}${cookies} pwdt_id=${encodeURIComponent($.UserName)};${$.uid ? ' sid='+$.uid+';' : ''} shshshfpv=;`,
    "Origin": "https://wbbny.m.jd.com",
    ...
    $.post(url, async (err, resp, data) => {
      try {
        ...
        if(res && typeof res === 'object' && res.toast){
          console.log(res.toast.subTitle)
        }else{
          console.log(data)
        }
      } catch (e) {
